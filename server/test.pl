...
sub print_array{
	...
	if($files =~ m/(.*)\.xls$/){
	    ...
	    my $ind_ann_output = $dir."x/".$file_prefix."_sum_".$ind_output;
	    my $tab_ann_output = $dir."x/".$file_prefix."_tab_".$ind_output;
	    my $full_ann_output = $dir."x/".$file_prefix."_full_".$full_output;
	    my $write_xl_path = "";
