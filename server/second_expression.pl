...
my $m450_dir = "/home/steve/Desktop/MA_core_download/backup/illumina backup/DataOutput/Illumina sheet/work sheet/baylin/test-exp/";
...
#	    my $two = $row[0]."\t".$row[1]."\t".$row[2]."\t".$row[4]."\t".$row[33]."\t".$row[34]."\t".$row[35]."\t".$row[36]."\t".$row[5]."\t".$row[6];
	    my $two = $row[0]."\t".$row[1]."\t".$row[2]."\t".$row[4]."\t".$row[34]."\t".$row[35]."\t".$row[36]."\t".$row[37]."\t".$row[5]."\t".$row[6];
	    print "TWO: $two\n";
...
#    print $exp."\n";
	...
	while(my $line = <FI>){
	    ...
	    #orig 33/34
	    push(@shipdate_ann, $row[34]);
	    print "33: $row[34]\n";
	    push(@usedate_ann, $row[35]);
	    print "34: $row[35]\n";

#	    push(@arraytype_ann, $row[35]);
#	    push(@bc_ann, $row[36]);
	    push(@arraytype_ann, $row[36]);
	    push(@bc_ann, $row[37]);
	    print "36: $row[36]\n";
	    print "37: $row[37]\n";
...
#	print H "PI Name 	User Name	REFERENCE\tSample Send Date	Sample Type	Sample Name	Sample Description	Cell No.	Send Genomic DNA  Concentration (ng/µl)	Volume (ul)	Yield (µg)                  	Date Process	Genomic DNA Concentration (ng/µl)	Vol (ul)	Yield (ug)	OD 260/280	OD 260/230	Run Agarose gel Loading Conc. (ng/ul)	Result of gel	Send SsDNA Con.c (ng/ul)	Vol (ul)	Yield (ng)	Input Genimic DNA (ug)	SsDNA Con.c (ng/ul)	Vol (ul)	Yield (ng)	OD 260/280	OD 260/230	Sample Label	InputSsDNA  For Amplify(ul)	Plate Position   #	Hyb Temp & Speed	Hyb Time	Ship-in Date	Use Date	Array Type	 Array Bar Code	Array position	Name/Bar code/position	Batch Code	Kit Serial Number	\n";
	...
	print H "PI Name 	User Name	REFERENCE	Sample Send Date	Sample Type	Sample Name	Sample Description	Cell No.	Send Concentration (ng/µl)	Volume (ul)	Yield (µg)                  	Date Process	Islation Protocol	Concentration (ng/µl)	Vol (ul)	Yield (ug)	OD 260/280	OD 260/230	Loading Conc. (ng/ul)	read ng/ul	28s/18s	RIN	Sample Label	Input For Amplify (ug)	Date RT	cRNA Conc. (ng/ul)	  	Yield(ug) 	260/280	230/260	Input cRNA  (µg)	Plate Position   #	Hyb Temp & Speed	Hyb Time	Ship-in Date	Use Date	Array Type	 Array Bar Code	Array position 	name /bar code/position\n";
